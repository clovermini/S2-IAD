...
from .pretrained import is_pretrained_cfg, get_pretrained_cfg, download_pretrained,\
    list_pretrained_tags_by_model, download_pretrained_from_hf, list_pretrained
...
def load_state_dict(checkpoint_path: str, map_location='cpu', weights_only=True):
...
def load_checkpoint(model, checkpoint_path, strict=True):
    ...
    state_dict = load_state_dict(checkpoint_path, weights_only=False) # default True , LIFT False
    ...
    # LIFT To accomodate an old version module naming: rename keys starting with "text_decoder" to "mlp_projector"
    new_state_dict = {}
...
def create_customer_model(
    ...
    preprocess_cfg = asdict(PreprocessCfg())
    has_hf_hub_prefix = model_name.startswith(HF_HUB_PREFIX)
    print('has_hf_hub_prefix ', has_hf_hub_prefix)
    if has_hf_hub_prefix:
        model_id = model_name[len(HF_HUB_PREFIX):]
        checkpoint_path = download_pretrained_from_hf(model_id, cache_dir=cache_dir)
        config = _get_hf_config(model_id, cache_dir)
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        model_pre = custom_load_openai_model(
            name = model_name,
            precision=precision,
            device=device,
            jit=jit,
        ...
        if pretrained_image:
            if is_timm_model:
                ...
                assert False, 'pretrained image towers currently only supported for timm models'
        print('is_timm_model ', is_timm_model)
        # cast_dtype set for fp16 and bf16 (manual mixed-precision), not set for 'amp' or 'pure' modes
        cast_dtype = get_cast_dtype(precision)
