...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, return_attn=False) -> Tensor:
        ...
        def attn_residual_func(x: Tensor, return_attn=False) -> Tuple[Tensor, Tensor]:
            out = self.attn(self.norm1(x), return_attn=return_attn)
            return out
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            # print('block .... return_attn ', return_attn)
            out = attn_residual_func(x, return_attn=return_attn)
            if return_attn:
                out, attn = out
            x = x + self.ls1(out)
...
class NestedTensorBlock(Block):
    def forward_nested(self, x_list: List[Tensor], return_attn=False) -> List[Tensor]:
        ...
        if self.training and self.sample_drop_ratio > 0.0:
            ...
            def attn_residual_func_old(x: Tensor, attn_bias=None) -> Tensor:
                return self.ls1(self.attn(self.norm1(x), attn_bias=attn_bias))
            
            def attn_residual_func(x: Tensor, attn_bias=None, return_attn=False) -> Tuple[Tensor, Tensor]:
                out = self.attn(self.norm1(x), attn_bias=attn_bias, return_attn=return_attn)
                return out
            ...
            out = attn_residual_func(x, attn_bias=attn_bias, return_attn=return_attn)
            if return_attn:
                out, attn = out
            x = x + self.ls1(out)
            x = x + ffn_residual_func(x)
    ...
    def forward(self, x_or_x_list, return_attn=False):
        # print('NestedTensorBlock return_attn ', return_attn, ' -- ', isinstance(x_or_x_list, Tensor))
        if isinstance(x_or_x_list, Tensor):
            return super().forward(x_or_x_list, return_attn=return_attn)
        elif isinstance(x_or_x_list, list):
            if not XFORMERS_AVAILABLE:
                raise AssertionError("xFormers is required for using nested tensors")
            return self.forward_nested(x_or_x_list, return_attn=return_attn)
        else:
